...
- Suffix every JPA entity class with `Entity`, such as `ReceiptEntity` or
  `ReceiptLineEntity`. Repositories operate on these persistence types.
...
- Services own all entity operations: creation, lookup, validation, field
  ...
  deletion. Let the database populate fields configured as database-generated.
  Use entity getters and setters within service operations; keep transaction
  boundaries and persistence calls in the service layer.
...
- Define service-layer domain models as Java records in `service.<domain>`.
  Use the plain domain name, such as `Receipt`, for the record corresponding
  to `ReceiptEntity`. These records are distinct from API DTOs.
- A domain record mirrors the entity's properties and persisted nullability
  requirements. Use Lombok `@NonNull` on required reference components; retain
  nullable components where absence is permitted. Include persisted values
  such as `id` and `createdAt`, even when they are generated by the database.
- Services accept operation models and emit domain records. Keep entities
  internal to service implementations and repositories; never accept or
  return entities through the public service API. Map entity relationships
  and collections to domain values without leaking JPA entity references.
- Map persisted entity state to the domain record before returning from the
  service operation. Ensure required generated values are available before
  constructing the record; a generated ID may be absent on a new entity but
  must be present on its emitted domain record.
...
- Persist a technical primary key as `Long`, but use a stable UUID represented
  ...
  persistence boundary. Domain records may carry the technical ID as a
  persisted property, but do not use it as the public API identity or couple
  domain operations to primary keys.
...
- Model domain-specific changes as named command types and explicit methods.
  ...
  `productEntity.setName(...)` inside it. The service API expresses the
  operation; the entity setter only assigns persistence state.
- Creation models primarily contain the mandatory, non-null caller-supplied
  fields required for that creation path. Include optional inputs only when
  the actual use case needs them. Exclude database-generated values such as
  the technical ID and `createdAt` when the database manages it; creation
  models are not copies of the complete entity or output record.
- Use multiple creation models when actual requirements provide distinct
  creation paths with different required inputs. For example, `CreateReceipt`
  and `CreateFailedReceipt` could each have a corresponding `createNew`
  overload. Do not add speculative creation variants.
- Name operation parameters after the action, such as `create`, `approve`,
  or `update`, rather than generic names such as `command` or `request`.
  Typical service signatures are:

  ```java
  public Receipt createNew(@NonNull CreateReceipt create)
  public Receipt approve(@NonNull ApproveReceipt approve)
  ```
